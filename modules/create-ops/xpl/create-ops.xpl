...
<p:declare-step xmlns:p="http://www.w3.org/ns/xproc" 
  ...
  <p:import href="../../link-checker/xpl/link-checker.xpl"/>
...
<!--      <p:empty/>-->
    ...
    <p:choose name="conditionally-check-hrefs">
      ...
      <p:when test="normalize-space($link-check) and not($link-check = 'false')">
        <p:output port="result" primary="true">
          <p:pipe port="result" step="check-links"/>
        </p:output>
        ...
        <tr:check-links name="check-links">
          <p:with-option name="only" select="$only"/>
          <p:with-option name="never" select="$never"/>
          <p:with-option name="debug-dir-uri" select="$debug-dir-uri" />
