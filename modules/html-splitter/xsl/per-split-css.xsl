...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
  ...
  <xsl:param name="restore-original-css-filenames" select="'true'"   as="xs:string"/>
  <xsl:param name="debug"                          select="'no'"     as="xs:string"/>
  <xsl:param name="final-pub-type"                 select="'EPUB3'"  as="xs:string"/>
  <xsl:param name="style-dir-name"                 select="'styles'" as="xs:string"/>
  ...
  <xsl:function name="css:relative-result-filename" as="xs:string">
    ...
    <xsl:variable name="prelim" as="xs:string" 
      select="if (matches($origin, '^(file://)internal$'))
              then concat($style-dir-name, '/internal-', $origin/../css:selector/@position ,'.css')
    ...
    <xsl:sequence select="if (matches($prelim, '^(file|https?):'))
                          then concat($style-dir-name, '/', replace($prelim, '^.+/', '')) 
                          else $prelim"/>
  </xsl:function>
