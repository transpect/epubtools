<p:declare-step xmlns:p="http://www.w3.org/ns/xproc" 
  ...
  <pre>calabash/calabash.sh 
    ...
    <p>Calabash seems to suppress some XSLT errors, for instance when a stylesheet is looping.
    Therefore it might be necessary to replace collection()[…] with document(…) in the XSL
    (alternative variable declarations are already included in the xsl file, commented out) 
    and run saxon from the command line, for example like this:</p>
...
saxon -xsl:epubtools/modules/html-splitter/xsl/html-splitter.xsl -s:$PRE_SPLIT -it:main \
  ...
  <p:identity name="strip-leading-non-elements">
    <p:documentation>Strip spurious text-only document nodes that sometimes occured before 
    the HTML document.</p:documentation>
  ...
  <p:try name="html-splitter-group">
    <p:documentation>You might need to comment out this p:try/p:catch and move name="html-splitter-group"
    to the followin p:group in order to facilitate debugging if there is an error in the splitter XSLT.
    In extreme cases, it might be necessary to invoke the XSLT directly. For instructions, see the 
    comments after the xsl:param instructions in html-splitter.xsl.</p:documentation>
    <p:group>
      <p:output port="files" primary="false">
        ...
        <p:when test="contains($css-handling, 'regenerated-per-split')">
          <p:output port="result" primary="true" sequence="true">
            <p:documentation>Each split HTML file, with an additional link to the individual CSS file if there are
              per-split CSS rules. Also, all of the newly generated CSS, with a base URI that matches 
            the corresponding HTML file’s, except for the file extension.</p:documentation>
            <p:pipe port="result" step="generate-css"/>
            <p:pipe port="secondary" step="insert-individual-css-link"/>
          </p:output>
          <p:output port="unused-css-resources">
            <p:documentation>A css:css document containing XML representations of unused @font-face at rules.</p:documentation>
            <p:pipe port="result" step="unused-css-resources"/>
          </p:output>
          <p:xslt name="per-split-css-xml-representations">
            <p:documentation>Primary output: the new, reduced common CSS. 
              Secondary port: individual CSS files if applicable.
              Also on secondary port: A file named 'unused-css-resources.xml'</p:documentation>
          ...
          <p:xslt name="insert-individual-css-link" template-name="main">
            <p:documentation>Will insert links for per-split css.
            Has side effect: svg namespace fixup.</p:documentation>
            <p:input port="parameters"><p:empty/></p:input>
            <p:input port="stylesheet">
      ...
      <p:for-each name="store-chunks">
        ...
        <p:variable name="chunk-file-uri" select="replace(base-uri(/*), 'chunks/', 'epub/OEBPS/')">
          <p:documentation>base-uri(/*) instead of base-uri() because we set the base uri of the primary
          CSS by adding an xml:base attribute.</p:documentation>
        </p:variable>
        ...
        <p:choose xmlns:epub="http://www.idpf.org/2007/ops">
          <p:when test="$target = 'EPUB3' and matches(base-uri(), 'nav\.xhtml$') and (normalize-space($html-subdir-name))">
            <p:documentation>Brute force link correction for the generated landmarks nav that will be stored to 
              OEBPS even when the remainder of the HTML is stored to a subdir.</p:documentation>
      ...
      <p:for-each name="signal-splitting-error">
        <p:documentation>The presence of an orig.txt is an indicator that the split text 
        differs from the original text. We’ll raise an error. We don’t do it immediately
        within the split step because we want to store the results first so that you can
        do forensics.</p:documentation>
        ...
        <p:add-attribute match="/html:p/html:a[1]" name="orig-txt-url" attribute-name="href" cx:depends-on="store-debug">
          ...
          <p:input port="source">
            <p:inline>
              <p xmlns="http://www.w3.org/1999/xhtml">The after-split text differs from the pre-split text. This typically
              occurs when there is bare text content on the same level as headings.
        ...
        <p:add-attribute match="/html:p/html:a[2]" name="chunks-txt-url"
          attribute-name="href">
      ...
      <p:wrap-sequence wrapper="document" wrapper-namespace="http://xmlcalabash.com/ns/extensions"
        wrapper-prefix="cx">
        <p:input port="source">
