...
<p:declare-step xmlns:p="http://www.w3.org/ns/xproc" 
  ...
  <p:output port="report" primary="false" sequence="true">
    ...
    <p:pipe port="result" step="missing-xml-base"/>
  </p:output>
  <p:output port="html">
  ...
  <p:option name="create-a11y-meta" select="'true'" cx:type="xs:string" required="false">
    ...
    <p:when test="empty(/*/@xml:base) (:matches(/*/@local-href, '\.xpl\.html$'):)">
      <p:try>
        <p:group>
          ...
          <p:error code="epub:BASE01" name="throw-error">
            <p:input port="source">
              <p:pipe port="result" step="msg3"/>
            </p:input>
          </p:error>
        </p:group>
